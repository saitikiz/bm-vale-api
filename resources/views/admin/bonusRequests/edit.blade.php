...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.bonusRequest.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.bonus-requests.update", [$bonusRequest->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="uuid">{{ trans('cruds.bonusRequest.fields.uuid') }}</label>
                <input class="form-control {{ $errors->has('uuid') ? 'is-invalid' : '' }}" type="text" name="uuid" id="uuid" value="{{ old('uuid', $bonusRequest->uuid) }}" required>
                @if($errors->has('uuid'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('worker') ? 'is-invalid' : '' }}" name="worker_id" id="worker_id">
                    @foreach($workers as $id => $entry)
                        <option value="{{ $id }}" {{ (old('worker_id') ? old('worker_id') : $bonusRequest->worker->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('customer_username') ? 'is-invalid' : '' }}" type="text" name="customer_username" id="customer_username" value="{{ old('customer_username', $bonusRequest->customer_username) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('customer_code') ? 'is-invalid' : '' }}" type="text" name="customer_code" id="customer_code" value="{{ old('customer_code', $bonusRequest->customer_code) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('customerid') ? 'is-invalid' : '' }}" type="text" name="customerid" id="customerid" value="{{ old('customerid', $bonusRequest->customerid) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('source') ? 'is-invalid' : '' }}" name="source" id="source" required>
                    ...
                    @foreach(App\Models\BonusRequest::SOURCE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('source', $bonusRequest->source) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('ip') ? 'is-invalid' : '' }}" type="text" name="ip" id="ip" value="{{ old('ip', $bonusRequest->ip) }}">
                @if($errors->has('ip'))
                    <div class="invalid-feedback">
                        {{ $errors->first('ip') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status">
                    ...
                    @foreach(App\Models\BonusRequest::STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status', $bonusRequest->status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('status_reason') ? 'is-invalid' : '' }}" type="text" name="status_reason" id="status_reason" value="{{ old('status_reason', $bonusRequest->status_reason) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('note') ? 'is-invalid' : '' }}" type="text" name="note" id="note" value="{{ old('note', $bonusRequest->note) }}">
                @if($errors->has('note'))
                    <div class="invalid-feedback">
                        {{ $errors->first('note') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('locked_at') ? 'is-invalid' : '' }}" type="text" name="locked_at" id="locked_at" value="{{ old('locked_at', $bonusRequest->locked_at) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('retry_count') ? 'is-invalid' : '' }}" type="number" name="retry_count" id="retry_count" value="{{ old('retry_count', $bonusRequest->retry_count) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('last_error') ? 'is-invalid' : '' }}" name="last_error" id="last_error">{{ old('last_error', $bonusRequest->last_error) }}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('site') ? 'is-invalid' : '' }}" name="site_id" id="site_id">
                    @foreach($sites as $id => $entry)
                        <option value="{{ $id }}" {{ (old('site_id') ? old('site_id') : $bonusRequest->site->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('site'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('bonus') ? 'is-invalid' : '' }}" name="bonus_id" id="bonus_id">
                    @foreach($bonuses as $id => $entry)
                        <option value="{{ $id }}" {{ (old('bonus_id') ? old('bonus_id') : $bonusRequest->bonus->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('bonus'))
                    <div class="invalid-feedback">
